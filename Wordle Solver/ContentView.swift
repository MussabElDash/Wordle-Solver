...
struct ContentView: View {
    ...
    private let bundledOpeners: [String] = loadWords(named: "openers")
    ...
    var body: some View {
        NavigationView {
            VStack(spacing: 16) {
                ...
                List {
                    // History = all completed guesses (exclude the current input row)
                    ...
                    Section("Top Suggestions") {
                        ForEach(suggestions) { s in
                            ...
                            .onTapGesture {
                                ...
                                // optional: reset colors for the new guess input
            ...
            .onAppear {
                openers = loadWords(named: "openers")
    ...
    private func sanitizeCurrentWord() {
        var w = rows[rows.count - 1].word.lowercased()
        w = w.filter { $0 >= "a" && $0 <= "z" }
        if w.count > 5 { w = String(w.prefix(5)) }
        rows[rows.count - 1].word = w
    }
    ...
    private func undo() {
        // Simple undo: rebuild from scratch using all but last completed guess
        guard rows.count > 1 else { return }
        rows.removeLast() // remove current empty row
        
        // remove last completed
        let completed = Array(rows.dropLast())
        rows = [GuessRow()]
        solver.reset()
        ...
        for r in completed {
            ...
            solver.apply(guess: r.word, patternCode: code)
        }
    ...
    private func computeKnownInfo() -> WordleSolver.KnownInfo {
        ...
        for r in rows.dropLast() {
            ...
            for i in 0..<5 {
                if r.colors[i] == 2 {          // green
                    greens[i] = letters[i]
                    present.insert(letters[i])
                } else if r.colors[i] == 1 {   // yellow
                    present.insert(letters[i])
                }
            }
        }
        ...
        for r in rows.dropLast() {
            ...
            for i in 0..<5 {
                ...
                if r.colors[i] != 0 { // yellow or green
        ...
        // If a letter is present, it cannot be absent
    ...
    private func recomputeSuggestions() {
        ...
        if solver.candidateCount == solver.answers.count && rows.count == 1 && (rows.first?.word.isEmpty ?? true) {
            // Build Suggestion objects with placeholder metrics (optional)
